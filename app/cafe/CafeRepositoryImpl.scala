...
class CafeRepositoryImpl @Inject()(
  ...
  def mkCafeEntity(rs: WrappedResultSet): Cafe =
    Cafe(
      id = rs.get("id"),
      name = rs.get("name"),
      coodinate = Coodinate(rs.get("latitude"), rs.get("longitude")),
  ...
  override def findAll(): Future[Seq[Cafe]] =
    Future {
      sql"""
        select
          c.cafe_id as id, c.latitude as latitude, c.longitude as longitude, c.name as name,
          group_concat(distinct i.url) as urls, avg(r.value) as averaged_value
        from cafes as c
